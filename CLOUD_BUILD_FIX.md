...
3. Under **Configuration**:
   ...
   - Set **Cloud Build configuration file location** to: **`cloudbuild.yaml`**  
     (The file is in the repo root, so use just `cloudbuild.yaml`, not `impactflow-os/cloudbuild.yaml`)
4. **Save**.
5. Run the build again.

After this, the build will use `cloudbuild.yaml`, which already sets `logging: CLOUD_LOGGING_ONLY`, so the error should stop.
...
- If **Build configuration** = **Cloud Build configuration file** → confirm the path is exactly **`cloudbuild.yaml`** (the file is in the repo root).
