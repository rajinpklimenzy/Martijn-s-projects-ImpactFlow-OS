...
import { 
  ...
  History, Info, MessageSquare, PieChart, UserPlus, Search, Sparkles, Edit2, RefreshCw, Mail, BookOpen
} from 'lucide-react';
...
// Playbook Instance Card Component - fetches completions to calculate accurate progress
const PlaybookInstanceCard: React.FC<{ instance: any; onView: () => void }> = ({ instance, onView }) => {
  ...
  return (
    ...
    >
      <div className="flex items-start justify-between mb-3">
          ...
          <div>
            ...
            {instance.templateSnapshot?.version && (
              <p className="text-xs text-slate-400 mt-0.5">
                Version {instance.templateSnapshot.version}
              </p>
            )}
          </div>
        </div>
        <div className="flex items-center gap-2">
          <span className={`px-2 py-1 rounded-lg text-xs font-semibold ${
            ...
            {instance.status === 'completed' ? 'Completed' : 'Active'}
          </span>
        </div>
      </div>
      ...
      <button
        onClick={onView}
        className="w-full px-4 py-2 bg-indigo-50 hover:bg-indigo-100 text-indigo-600 text-xs font-semibold rounded-xl transition-colors flex items-center justify-center gap-2"
      >
...
const Pipeline: React.FC<{ onNavigate: (tab: string) => void; onNewDeal: (stage?: string) => void; currentUser?: any }> = ({ onNavigate, onNewDeal, currentUser }) => {
  ...
  const [activeDetailTab, setActiveDetailTab] = useState<'overview' | 'activity' | 'stakeholders' | 'playbooks'>('overview');
  ...
  // Fetch playbook instances for selected deal
  const { data: dealPlaybookInstances = [], isLoading: isLoadingPlaybooks } = usePlaybookInstances({
            ...
            <div className="p-8 border-b border-slate-100 bg-slate-50/50">
              ...
              <div className="flex bg-white p-1 rounded-2xl border border-slate-100 w-fit">
                {[
                  ...
                  { id: 'playbooks', label: 'Playbooks', icon: <BookOpen className="w-4 h-4" /> },
                ].map(tab => (
                  <button
                    key={tab.id}
                    onClick={() => { 
              ...
              {activeDetailTab === 'playbooks' && (
                <div className="space-y-8 animate-in fade-in slide-in-from-left-4 duration-300">
                  <div className="flex justify-between items-center">
                    ...
                    >
                      <Plus className="w-3 h-3" /> Attach Playbook
                    </button>
                  </div>
                  ...
                  ) : dealPlaybookInstances.length > 0 ? (
                    <div className="space-y-3">
                      {dealPlaybookInstances.map((instance: any) => {
                        return <PlaybookInstanceCard key={instance.id} instance={instance} onView={() => setViewingPlaybookInstanceId(instance.id)} />;
                      })}
                    </div>
                  ) : (
                    <div className="p-12 border-2 border-dashed border-slate-100 rounded-[40px] text-center">
                      <BookOpen className="w-12 h-12 text-slate-200 mx-auto mb-4" />
                      <p className="text-xs text-slate-400 font-medium">No playbooks attached to this deal yet.</p>
                    </div>
                  )}
                </div>
              )}
