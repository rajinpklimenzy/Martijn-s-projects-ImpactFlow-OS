...
const Roadmap: React.FC<{ currentUser: any, onNavigate?: (tab: string) => void }> = ({ currentUser, onNavigate }) => {
  ...
  const handleBulkDeleteItems = async () => {
    if (!isAdmin) {
      showError('Only admins can delete entries');
      return;
    }
    ...
    try {
      ...
      const deletePromises = targetIds.map(id => apiDeleteFeedback(id));
      await Promise.all(deletePromises);
      
      // Update state
      setItems(prev => prev.filter(item => !targetIds.includes(item.id)));
      
      // Close detail view if selected item was deleted
      if (selectedItem && targetIds.includes(selectedItem.id)) {
        setSelectedItem(null);
      }
  ...
  return (
    <div className="space-y-8 animate-in fade-in duration-500 pb-32 relative h-full">
            ...
            <>
              ...
              {filteredItems.length > 0 && (
                <div className="flex items-center gap-3 pb-2">
                  ...
                  />
                  <span className="text-sm font-semibold text-slate-600 cursor-pointer" onClick={(e) => {
                    e.stopPropagation();
                    selectAllItems();
                  }}>
                    Select All Entries
                  ...
                  return (
                    <div 
                      ...
                      onClick={() => {
                        ...
                        setOpenMenuId(null); // Close menu when clicking card
                      }}
                      className={`bg-white rounded-[32px] border ${isSelected ? 'border-indigo-400 bg-indigo-50/30' : 'border-slate-200'} p-8 shadow-sm hover:border-indigo-300 hover:shadow-xl transition-all group flex flex-col relative cursor-pointer active:scale-[0.98] hover:scale-[1.02] overflow-visible`}
                    >
                    <div className="flex justify-between items-start mb-6">
                       ...
                       <div className="flex items-center gap-2">
                         ...
                         <div className="flex items-center gap-1.5" onClick={(e) => e.stopPropagation()}>
                           {/* Checkbox */}
                           <div 
                             className={`${isSelected ? 'opacity-100' : 'opacity-0 group-hover:opacity-100'} transition-opacity flex items-center`}
                           >
                             <label className={`px-2 py-1 rounded-lg transition-all border flex items-center justify-center cursor-pointer ${isSelected ? 'bg-indigo-50 border-indigo-200' : 'bg-slate-50 border-slate-200 hover:bg-indigo-50/50 hover:border-indigo-200'}`}>
                               <input 
                                 type="checkbox" 
                                 className="rounded border-slate-300 text-indigo-600 focus:ring-indigo-500 w-4 h-4 cursor-pointer"
                                 checked={isSelected}
                             ...
                             <>
                               {item.status !== 'in-progress' && (
                                 <button
                                   ...
                                   className="px-2 py-1 hover:bg-blue-50 rounded-lg text-blue-600 transition-all border border-blue-100 hover:border-blue-200 flex items-center justify-center"
                               ...
                               <button
                                 ...
                                 className="px-2 py-1 hover:bg-emerald-50 rounded-lg text-emerald-600 transition-all border border-emerald-100 hover:border-emerald-200 flex items-center justify-center"
                                 title="Mark as Done"
                               >
                               ...
                               <button
                                 ...
                                 className="px-2 py-1 hover:bg-slate-100 rounded-lg text-slate-500 transition-all border border-slate-200 hover:border-slate-300 flex items-center justify-center"
                                 title="Archive"
                               >
                           ...
                           {/* Admin Status Menu (Three-dot button) */}
                           {isAdmin && (
                             <div className="relative">
                               <button 
                                 ...
                                 className={`px-2 py-1 hover:bg-slate-50 rounded-lg text-slate-400 transition-all border border-slate-200 hover:border-slate-300 flex items-center justify-center ${openMenuId === item.id ? 'opacity-100' : 'opacity-0 group-hover:opacity-100'}`}
                                 title="More options"
                               >
                               ...
                               {openMenuId === item.id && (
                                 <>
                                   ...
                                   />
                                   <div className="absolute right-0 top-full mt-2 w-52 bg-slate-900 rounded-2xl shadow-2xl overflow-hidden z-[60] animate-in fade-in zoom-in-95 duration-200">
                                     ...
                                     <div className="overflow-hidden rounded-b-2xl">
                                       {(['planned', 'in-progress', 'done', 'postponed', 'canceled'] as FeedbackItem['status'][]).map(s => (
                                         <button 
                                           ...
                                           className={`w-full px-4 py-3 text-left text-[10px] font-black uppercase tracking-widest transition-colors ${item.status === s ? 'text-indigo-400 bg-white/5' : 'text-slate-300 hover:bg-white/5'}`}
                                         >
                                           Set {s}
                                         </button>
                                       ))}
                                       <button 
                                         ...
                                         className="w-full px-4 py-3 text-left text-[10px] font-black uppercase tracking-widest text-red-400 border-t border-white/5 hover:bg-red-500 hover:text-white transition-all"
                                       >
                                         Delete Terminal
                                       </button>
                                     </div>
      ...
      {activeSection === 'registry' && (
        <div className="mt-10 space-y-4 pb-12">
          ...
          <p className="text-slate-500 text-sm max-w-2xl">
            These confirmed feature requests are currently{' '}
            ...
            <div className="bg-white border border-slate-200 rounded-3xl p-5 flex flex-col gap-3">
              <div className="flex items-center justify-between">
                <div className="flex items-center gap-3">
                  ...
                  <div>
                    <p className="text-xs font-black uppercase tracking-widest text-slate-500">Feature Request</p>
                    <h3 className="text-sm font-black text-slate-900">Business Card Scanner</h3>
                  </div>
                </div>
              ...
              <p className="text-xs text-slate-500">
                Scan a business card and create contact and company information directly inside the application.
              </p>
              <button
                disabled
            ...
            <div className="bg-white border border-slate-200 rounded-3xl p-5 flex flex-col gap-3">
              <div className="flex items-center justify-between">
                <div className="flex items-center gap-3">
                  ...
                  <div>
                    <p className="text-xs font-black uppercase tracking-widest text-slate-500">Feature Request</p>
                    <h3 className="text-sm font-black text-slate-900">LinkedIn Scanner</h3>
                  </div>
                </div>
                <span className="px-2 py-0.5 rounded-full text-[9px] font-black bg-indigo-50 text-indigo-600">
                  Planned
                </span>
              </div>
              <p className="text-xs text-slate-500">
                Scan a LinkedIn account and create contact and company information directly inside the application.
              </p>
              <button
                disabled
                className="self-start px-4 py-2 rounded-full text-[10px] font-black uppercase tracking-widest bg-slate-100 text-slate-400 cursor-not-allowed flex items-center gap-2"
            ...
            <div className="bg-white border border-slate-200 rounded-3xl p-5 flex flex-col gap-3">
              <div className="flex items-center justify-between">
                <div className="flex items-center gap-3">
                  ...
                  <div>
                    <p className="text-xs font-black uppercase tracking-widest text-slate-500">Feature Request</p>
                    <h3 className="text-sm font-black text-slate-900">Source Signals</h3>
                  </div>
                </div>
          ...
          <div className="bg-slate-900 text-white rounded-3xl shadow-2xl px-8 py-4 flex items-center gap-8 border border-white/10 ring-4 ring-indigo-500/10">
            ...
            <div className="flex items-center gap-3">
              <button 
                ...
                className="px-5 py-2 bg-red-600 hover:bg-red-700 rounded-xl text-xs font-black uppercase tracking-widest transition-all flex items-center gap-2 disabled:opacity-50 disabled:cursor-not-allowed"
              >
                {isBulkDeletingItems ? (
      ...
      {bulkDeleteConfirmOpen && selectedItemIds.length > 0 && (
        <div className="fixed inset-0 z-[130] flex items-center justify-center p-4 bg-slate-900/60 backdrop-blur-sm animate-in fade-in duration-300">
          <div className="bg-white rounded-[32px] w-full max-w-md shadow-2xl overflow-hidden animate-in zoom-in-95 duration-200">
            <div className="p-8 border-b border-slate-100">
              <div className="flex items-center gap-4 mb-6">
                ...
                <div>
                  <h3 className="text-xl font-black text-slate-900">
                    Delete {selectedItemIds.length} Entr{selectedItemIds.length > 1 ? 'ies' : 'y'}?
                  </h3>
                  <p className="text-xs text-slate-400 font-medium mt-1">
                    This action cannot be undone. The entries will be permanently removed from the registry.
                  </p>
                </div>
              </div>
              <div className="p-4 bg-slate-50 rounded-2xl border border-slate-200 max-h-48 overflow-y-auto">
                {items.filter(item => selectedItemIds.includes(item.id)).slice(0, 5).map(item => (
